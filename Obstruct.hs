...
_decompUnder ctd ctd' (heur,me) facts = foldr (\(f,left,len) (h,acc) ->
                                               if len <= me
                                               then let (nh,res) = (heur,generateLessThan h ctd ctd' f)
...
decompositions ctd ctd' cg' heuristics = 
       ...
       where factors = getAllFactors cg'
             (heuristics',withlts) = _decompUnder ctd ctd' (heuristics,numEdgeFactors (getPath cg')) factors
...
thm114 a c cg' = obstruct (Polydisk a 1) (Ellipsoid c c) cg' thm114Heuristics
...
squeezeLower heuristics ctd ctds' cg' midc coffs =
                             let cs = offsWithCTDs midc coffs ctds'
                                 decomps = decompositions ctd (snd $ last cs) cg' heuristics 
